...
for i in range(0, 116, 1):
	trainingSet.addSample((normalized_data[i], normalized_data[i+1], normalized_data[i+2]), (normalized_data[i+3]))#116 tuples
...
for i in range(119, 163, 1):
	ds.addSample((normalized_data[i], normalized_data[i+1], normalized_data[i+2]), (normalized_data[i+3]))#44 tuples

# print "dataset"
# for inpt, target in ds:
# 	print inpt, target

net = buildNetwork(3, 2, 1, bias = True, hiddenclass = SigmoidLayer)
...
y = net.activate([0.10031611360730924, 0.13198268361816343, 0.239516498322237])
print y
